<template>
  <div>
    <DashboardHeader header-name="Fifth Section Edit" />
    <div class="main-dashboard-content-form-data">
      ...
      <div class="mt-5 mb-3">
        <button class="btn btn-primary custom-btn" @click="updateFifthSection">Submit</button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  mounted() {
    this.getFifthSection()
  },
  methods: {
    async getFifthSection() {
    ...
    async updateFifthSection() {
      ...
      this.$store.commit('loaders/show')
      try {
          const response = await this.$api.post(`/home-page-fifth-section`, {heading:this.heading, paragraph:this.paragraph} ) // eslint-disable-line
          this.error = false
          this.errorMessage = ''
