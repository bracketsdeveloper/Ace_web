<template>
  <div>
    <DashboardHeader header-name="Third Section Edit" />
    <div class="main-dashboard-content-form-data">
      ...
      <div class="mt-5 mb-3">
        <button class="btn btn-primary custom-btn" @click="updateThirdSection">Submit</button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async updateThirdSection() {
      ...
      this.$store.commit('loaders/show')
      try {
          const response = await this.$api.post(`/home-page-third-section`, {heading:this.heading, paragraph:this.paragraph} ) // eslint-disable-line
          this.error = false
          this.errorMessage = ''
